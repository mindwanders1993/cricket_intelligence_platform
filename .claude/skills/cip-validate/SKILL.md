...
- `milestone` will spend ~$1–2 in tokens. If `--dry` was passed, set `SKIP_AI=1` for the run. Otherwise confirm cost with `AskUserQuestion` if the mode was auto-selected (not user-explicit).
