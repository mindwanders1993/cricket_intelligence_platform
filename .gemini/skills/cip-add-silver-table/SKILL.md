...
`<PIPELINE>` is then used to resolve the exact files in Step 3:
- Job module: `src/cip/ingestion/jobs/build_silver_<PIPELINE>.py`
- DAG file: `orchestration/airflow/dags/ingest_people_and_names_silver.py` (for `people_and_names`) or `orchestration/airflow/dags/ingest_all_match_data_silver.py` + `ingest_two_day_match_data_silver.py` (for `match_data` — both DAGs share the same job module).
...
About to scaffold silver.<target>:
  ...
  5. Add a task to the relevant silver DAG(s):
        - people_and_names → orchestration/airflow/dags/ingest_people_and_names_silver.py
        - match_data       → orchestration/airflow/dags/ingest_all_match_data_silver.py
                              AND orchestration/airflow/dags/ingest_two_day_match_data_silver.py
...
**5. DAG task** — wire a `PythonOperator` calling the new job function into the appropriate silver DAG(s):
- `orchestration/airflow/dags/ingest_people_and_names_silver.py` (for `people_and_names`)
- `orchestration/airflow/dags/ingest_all_match_data_silver.py` AND `orchestration/airflow/dags/ingest_two_day_match_data_silver.py` (for `match_data` — both DAGs invoke `task_build_silver` in `cip.ingestion.jobs.build_silver_match_data`, so they pick up the new task automatically once the job module is updated).
Set the operator's dependency to run after the existing Silver tasks (`>>`).
...
Scaffold complete. Files added/modified:
  ...
  ✓ orchestration/airflow/dags/ingest_<scope>_<dataset>_silver.py — DAG task wired (both `all` and `two_day` for match_data)
