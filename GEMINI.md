...
- **Instructions:** See the Sections below for detailed developer commands, module layouts, and naming conventions.
...
This project uses `graphifyy` to maintain a knowledge graph of the codebase at `gemini-graphify-out/` with god nodes, community structure, and cross-file relationships.

Rules for Context Optimization:
- ALWAYS read `gemini-graphify-out/GRAPH_REPORT.md` before reading any source files, running grep/glob searches, or answering codebase questions. The graph is your primary map of the codebase and saves significant context tokens.
...
- After modifying code, run `poetry run graphify update . --output gemini-graphify-out` to keep the graph current (AST-only, no API cost).
...
User-invocable workflows in `.gemini/skills/` — use `activate_skill` tool to trigger. Each skill's `SKILL.md` is the source of truth for what it does and how it composes existing scripts/jobs.
...
All skills are project-scoped (committed in `.gemini/skills/`) so the toolkit travels with the repo.
