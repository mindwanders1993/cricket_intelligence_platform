...
How to work with Gemini CLI across every phase of development in this repo.

---

## Overview

This repo is configured with several layers that shape how Gemini behaves:

| Layer | File | Purpose |
|---|---|---|
| Default mode | `~/.gemini/settings.json` | Plan mode on by default — Gemini plans before executing |
| Permissions | `.gemini/settings.local.json` | Pre-approved CI/test commands (no prompt for these) |
| Hooks | `.gemini/settings.json` | Auto-nudge to check knowledge graph; auto-update graph after edits |
| Project memory | `~/.gemini/projects/.../memory/` | 13 files of accumulated project gotchas, loaded every session |
| Project skills | `.gemini/skills/` | 6 domain workflows (scaffold, inspect, validate, diagnose, etc.) |
| Architecture doc | `GEMINI.md` | Always in context — architecture, commands, working agreement |
...
Gemini will not execute anything without your approval. The flow is:

1. You describe a task
2. Gemini explores the codebase (read-only)
3. Gemini writes a plan to `.gemini/plans/`
...
`.gemini/settings.local.json` pre-approves a specific set of commands so you don't get prompted for routine CI operations:
...
Whenever Gemini is about to run `grep`, `rg`, `find`, `fd`, `ack`, or `ag`, it gets a message:
...
This nudges Gemini (and you) to check the knowledge graph before brute-forcing a file search. The graph has pre-extracted cross-file relationships, community clusters, and god nodes that raw grep can't surface.

### 2. Graph auto-update (PostToolUse)

After every `.py` file write or edit, Gemini automatically runs:
...
Project memory lives at `~/.gemini/projects/.../memory/` and is loaded into every session. It contains 13 files covering:
...
**To add to memory:** say "remember that..." and Gemini will save a new memory file. Memory persists across sessions.
...
Since `GEMINI.md` is always in context, Gemini can answer architecture questions without searching:
...
2. Gemini explores relevant code (read-only), then writes a plan.
3. Review the plan. Push back on anything that feels over-engineered.
4. Approve → Gemini executes.

The plan file lives at `.gemini/plans/`. You can re-read it mid-execution if you lose context.

### The 4 working agreement guardrails

These are in `GEMINI.md` and apply to every design conversation:

**1. Think before coding**
Gemini must state assumptions explicitly. If Gemini says "I'll assume `match_id` is unique at this grain" — verify it. Multi-wicket deliveries broke `fact_delivery` because a uniqueness assumption wasn't checked. Push back with: "have you verified that key is unique?"
...
**3. Surgical changes**
Gemini should touch only files the task requires. If the plan includes "I'll also improve adjacent code" or "while I'm here, I'll refactor X" — that's scope creep. Say "don't touch that, keep it minimal."

**4. Goal-driven execution**
Define success criteria before Gemini writes a single line. For each task type:
...
State these upfront. Don't let Gemini define them retroactively.
...
Review each diff. Every changed line should trace directly to your request. If lines are changing that you didn't ask about, ask Gemini why.
...
For milestone mode, Gemini confirms cost before running. Don't skip this confirmation.
...
**Asking Gemini to implement without stating assumptions.**
...
**Asking Gemini to "also improve" adjacent code.**
...
Milestone mode runs the full validation harness and costs ~$1-2 in LLM API calls. Gemini will ask for confirmation — answer deliberately.
