# orchestration/airflow/dags/ingest_all_match_data_silver.py
#
# DAG: ingest_all_match_data_silver
#
# Purpose:
#   Full-load Silver build for match data. Reads Bronze match_data Iceberg
#   table, runs incremental delete_and_insert for pending match_ids, and
#   writes to Silver Iceberg tables.
#
#   Triggered automatically by ingest_all_match_data_bronze, but can also
#   be run standalone to re-process Silver without re-downloading Bronze.
#
# Schedule: None — triggered by ingest_all_match_data_bronze or manually.
#
# Task graph:
#
#   check_infra
...
#
# Manual trigger:
#   airflow dags trigger ingest_all_match_data_silver
#   airflow dags trigger ingest_all_match_data_silver \
...
from cip.ingestion.jobs.full_load_match_data import task_build_silver
...
with DAG(
    dag_id="ingest_all_match_data_silver",
    description=(
        "Full-load Silver build: Bronze match_data → Silver Iceberg tables "
        "(incremental delete_and_insert by match_id)."
    ),
    start_date=datetime(2026, 5, 1),
    schedule=None,  # triggered by ingest_all_match_data_bronze or manual
    catchup=False,
    max_active_runs=1,
    default_args=_DEFAULT_ARGS,
    tags=["silver", "cricsheet", "full-load", "manual"],
    doc_md="""
## ingest_all_match_data_silver

Full-load Silver build for match data. Reads `bronze.match_data`, runs
incremental `delete_and_insert` for pending match_ids, and writes typed
Silver Iceberg tables.

Triggered automatically by `ingest_all_match_data_bronze` after DQ passes.
...
""",
    ...
    trigger_gold = TriggerDagRunOperator(
        ...
        trigger_dag_id=DagNames.INGEST_ALL_MATCH_DATA_GOLD,
